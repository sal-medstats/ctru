...
## R01_004,0,1,1,0
##
## Variables 'medication_a_o' and so on are therefore not listed and need adding.
master$lookups_fields <- rbind(master$lookups_fields,
                               c('Meds','','A','medication_a_o','Taking medication A'),
                               c('Meds','','A','medication_b_o','Taking medication B'),
                               c('Meds','','A','medication_c_o','Taking medication C'),
                               c('Meds','','A','medication_d_o','Taking medication D'))
## Read in the 'Lookups.csv' to this list
master$lookups <- read_prospect(file       = 'Lookups.csv',
	                            header     = TRUE,
                                sep        = ',',
                                dictionary = NULL)
## Similarly the levels for the binary indicators that are recorded as Type = Flag are
## NOT documented in the Lookups and therefore need adding.  These are always binary
## so are straightforward to add in the following manner...
master$lookups <- rbind(master$lookups,
                        c('Meds','','medication_a_o',0,'No'),
                        c('Meds','','medication_a_o',1,'Yes'),
                        c('Meds','','medication_b_o',0,'No'),
                        c('Meds','','medication_b_o',1,'Yes'),
                        c('Meds','','medication_c_o',0,'No'),
                        c('Meds','','medication_c_o',1,'Yes'),
                        c('Meds','','medication_d_o',0,'No'),
                        c('Meds','','medication_d_o',1,'Yes'))
```
...
|----------------------------|-----------------------------|--------------------------|
| `Error in file(file, "rt") : cannot open the connection` | The specified file can not be found | You are eitehr in the wrong directory, or you mis-typed the filename. |
